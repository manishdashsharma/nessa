...
function LightIntensity({ setBrightness }) {
  ...
  // Function to generate random heights for the bars
  ...
  const handleBarsClick = (e) => {
    ...
    setBrightness(newIntensity * 100); // Convert to percentage and pass to setBrightness
  };
      ...
      <div className="space-y-6">
        ...
        >
          <div className="flex justify-between items-center gap-1 h-full">
            {[...Array(21)].map((_, index) => {
              ...
              const isActive = intensity >= percentage; // Updated to intensity state
              const height =
                barHeights[index] ||
          ...
          <input
            ...
            max="1"
            step="0.01"
            value={intensity} // Updated to intensity state
            onChange={(e) => {
              const newIntensity = parseFloat(e.target.value);
              setIntensity(newIntensity); // Update the intensity state
              setBrightness(newIntensity * 100); // Convert to percentage and update brightness
            }}
        ...
        <div className="flex justify-between text-sm md:text-lg text-gray-500 font-medium">
          ...
          <span className="text-lg md:text-2xl font-bold text-gray-800">
            {Math.round(intensity * 100)}% {/* Updated to intensity state */}
          </span>
          <span>100%</span>
        </div>
      </div>
...
function ColorChangeController({ setColor }) {
  ...
  if (draggingAngle >= 0 && draggingAngle < 120) {
    // Yellow to Green (0° to 120°)
    mainColor = `hsl(63, 90.60%, 53.90%) 30%`;
    setColor("yellow");
  } else if (draggingAngle >= 120 && draggingAngle < 240) {
    // Green to Blue (120° to 240°)
    mainColor = `hsl(120, 100%, 50%) 50%`;
    setColor("green");
  } else if (draggingAngle >= 240 && draggingAngle < 360) {
    // Blue to Yellow (240° to 360°)
    mainColor = `hsl(212, 78%, 66%) 100%`;
    setColor("blue");
  }

  // Generate gradient stops for Yellow, Green, and Blue
  const gradientStops = [
    `hsl(63, 90.60%, 53.90%) 30%`, // Yellow
    `hsl(120, 100%, 50%) 50%`, // Green
    `hsl(212, 78%, 66%) 100%`, // Blue
  ].join(", ");

  return (
    <div className="w-full max-w-md bg-white p-3 rounded-xl ">
      <div className="text-center mb-2">
        <h2 className="text-lg md:text-2xl font-semibold  text-black flex items-center justify-center gap-3">
          Change Color
        </h2>
      </div>
      ...
      <div
        ref={dialRef}
        className="relative w-[70vw] sm:w-[250px] h-[70vw] sm:h-[250px] mx-auto p-[5vw] sm:p-[40px] bg-gray-100 rounded-full"
        onMouseMove={handleMouseMove}
        onTouchMove={handleTouchMove}
        onMouseDown={handleMouseDown}
        onTouchStart={handleMouseDown}
      >
        {/* Color ring with Yellow, Green, and Blue stops */}
        <div
        ...
        <div
          className="absolute inset-20 rounded-full transition-colors duration-200 border-[10px] border-gray-100"
          style={{
        ...
        <div
          ...
          style={{
            left: "50%",
            top: "50%",
            transform: `rotate(${draggingAngle}deg) translateY(-60px) rotate(-${draggingAngle}deg)`,
  ...
  return (
    <div>
      ...
      ></div>
          ...
          <div className="w-full  md:translate-y-[-5vw] flex gap-10 justify-center flex-wrap">
